...
function Workspace({
  ...
  const [leftSidebarWidth, setLeftSidebarWidth] = useState(() => {
    ...
    const width = Number(localStorage.getItem(STORAGE_KEYS.leftSidebarWidth));
    return Math.max(200, Math.min(420, width || DEFAULT_LEFT_WIDTH));
  });
  const [inspectorWidth, setInspectorWidth] = useState(() => {
    if (typeof window === "undefined") return DEFAULT_INSPECTOR_WIDTH;
    const width = Number(localStorage.getItem(STORAGE_KEYS.inspectorWidth));
    return Math.max(260, Math.min(520, width || DEFAULT_INSPECTOR_WIDTH));
  });
